...
public class PrivateKeyFactory {
    ...
    public ECKey getKey(String format, String data) throws Exception {
        if (format.equals(WIF_UNCOMPRESSED) || format.equals(WIF_COMPRESSED)) {
            DumpedPrivateKey pk = new DumpedPrivateKey(NetworkParams.getInstance().getCurrentParams(), data);
            return pk.getKey();
    ...
    private ECKey decodeMiniKey(String hex) throws Exception {
        ...
        try {
            String uncompressedAddress = uncompressedKey.toAddress(NetworkParams.getInstance().getCurrentParams()).toString();
            String compressedAddress = compressedKey.toAddress(NetworkParams.getInstance().getCurrentParams()).toString();
