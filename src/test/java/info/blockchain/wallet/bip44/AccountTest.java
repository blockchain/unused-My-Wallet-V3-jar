...
public class AccountTest {
    ...
    public void xpubstr() throws Exception {
        Account account = new Account(MainNetParams.get(), xpub);
        Assert.assertEquals(xpub,account.xpubstr());
    }

    @Test
    public void xprvstr() throws Exception {
        Account account = new Account(MainNetParams.get(), key, 0);
        Assert.assertEquals(xpriv,account.xprvstr());
    }

    @Test
    public void getId() throws Exception {
        Account account = new Account(MainNetParams.get(), xpub, 1);
        Assert.assertEquals(xpub,account.xpubstr());
        Assert.assertEquals(1,account.getId());

        account = new Account(MainNetParams.get(), xpub);
        Assert.assertEquals(xpub,account.xpubstr());
